...
function MySkillsPage() {
  ...
  const sendRequestname = async () => {
    const res = await axios
      .get(`http://localhost:4000/api/personal/user/634e940d5e10d9a518af8fb6`)
  ...
  const sendRequest = async () => {
    const res = await axios
      .get(`http://localhost:4000/api/about/user/634e940d5e10d9a518af8fb6`)
      .catch((err) => console.log(err));
    const data = await res.data;
  ...
  useEffect(() => {
    ...
    sendRequest().then((data) => setproject(data.project.aboutdetails));
  ...
  return (
   ...
   {perosnalsx &&
            perosnalsx.map((personal, index) => (
              <div>
        <LogoComponent  name={personal.firstname}  theme="LightTheme" />
        </div>
            ))}
